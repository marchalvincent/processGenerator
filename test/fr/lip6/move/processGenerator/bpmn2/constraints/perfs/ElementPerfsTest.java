...
import fr.lip6.move.processGenerator.bpmn2.constraints.impl.BpmnElementConstraintOcl;
...
public class ElementPerfsTest {
	...
	public void test(IStructuralConstraint constraint, String name, boolean bool) throws Exception {
		...
		checker = new StructuralConstraintChecker(constraint, EQuantity.EQUAL, 6);
		assertTrue(checker.check(process));
		...
		checker = new StructuralConstraintChecker(constraint, EQuantity.EQUAL, 4);
		assertTrue(checker.check(process));
		...
		checker = new StructuralConstraintChecker(constraint, EQuantity.EQUAL, 7);
		assertTrue(checker.check(process));
		
		process = BpmnBuilder.instance.getExampleForSESE2();
		checker = new StructuralConstraintChecker(constraint, EQuantity.EQUAL, 5);
		assertTrue(checker.check(process));
	...
	public void testOCL() throws Exception {
		...
		// Java
		IStructuralConstraint c1 = new BpmnElementConstraintOcl(EBpmnElement.TASK);
		this.test(c1, "OCL", false);
		this.test(c1, "OCL", true);
		
		IStructuralConstraint c2 = new BpmnElementConstraint(EBpmnElement.TASK);
