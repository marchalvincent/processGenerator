...
public class BpmnChangePatternHelper {
	...
	/**
	 * Renvoie une {@link ExclusiveGateway} ou une {@link InclusiveGateway} diverging tirée au hasard parmis celles du
	 * process passé en argument.
	 * 
	 * @param process
	 *            le {@link BpmnProcess} à parcourir.
	 * @param rng
	...
	public Gateway getRandomConditionalGatewayDiverging(BpmnProcess process, Random rng) throws GeneticException {
		...
		List<Gateway> liste = BpmnFilter.gatewayByType(classes, process.getProcess().getFlowElements(),
				GatewayDirection.DIVERGING);
	...
	private void removeUselessParallelGateway(BpmnProcess process) {
		...
		List<ParallelGateway> listeDiverging = BpmnFilter.byType(ParallelGateway.class, process.getProcess().getFlowElements(),
				GatewayDirection.DIVERGING);
			...
			if (parallelGateway.getOutgoing().size() == 1) {
				...
				if (parallelConverging.getIncoming().size() != 1)
					System.err.println(this.getClass().getSimpleName()
							+ " : The parallelGateway have more than 1 incoming sequence flow.");
	...
	/**
	 * Cette méthode se charge de supprimer les parallel gateway de nouveaux threads qui ne servent à rien ou qui ne
	 * sont pas logiques.
	 * 
	 * @param process
	 *            le {@link BpmnProcess} à nettoyer.
	...
	private void cleanNewThread(BpmnProcess process, ParallelGateway parallelGateway) {
		...
		if (parallelGateway.getOutgoing().size() > 1) {
			...
			/*
			 * on vérifie le cas où un des arcs arrive directement à un EndEvent. C'est illogique, car le process se
		...
		if (parallelGateway.getOutgoing().size() == 1) {
			...
			if (parallelGateway.getIncoming().size() != 1)
				System.err.println("Error, the parallel gateway has not only 1 incoming edge : "
						+ parallelGateway.getIncoming().size());
	...
	private void removeUselessConditionalGateway(BpmnProcess process) {
		...
		List<Gateway> listeDivergente = BpmnFilter.gatewayByType(classes, process.getProcess().getFlowElements(),
				GatewayDirection.DIVERGING);
